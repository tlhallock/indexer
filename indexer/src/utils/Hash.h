...
std::unique_ptr<DataInputStream> read_file_index(const char *original_file_name, const char *key, bool returnNonExists=false);
...
std::unique_ptr<DataInputStream> read_word_index(const char *key, bool returnNonExists=false);
std::unique_ptr<DataOutputStream> write_word_index(const char *key);
void remove_word_index(const char *key);

std::unique_ptr<DataInputStream> read_super_string_index(const char *key, bool returnNonExists=false);
std::unique_ptr<DataOutputStream> write_super_string_index(const char *key);
void remove_super_string_index(const char *key);

std::unique_ptr<DataInputStream> read_substring_index(const char *key, bool returnNonExists=false);
